...
feature_names = ['Name (Word Count)', 'Blurb (Grade Level)', 'Category', 'Country',
                 'State', 'City', 'Goal in $', 'Duration']
X = df.iloc[:, 1:-2]
...
def permutation_importance(model, X, y, repititions=5):
    ...
    for feature in X.columns:
        ...
        permutation_importances[feature] = reference_score - (1/repititions)* \
        np.sum(feature_permutation_scores)
    return permutation_importances

#------------- Classifier -------------#
X_train, X_test, y_train, y_test = train_test_split(X, y_cla, test_size=0.1,
                                                    random_state=42)
...
plt.barh(feature_names[np.argsort(pi_cla_scores)],
         pi_cla_scores[np.argsort(pi_cla_scores)])
plt.xlabel('Permutation Importance')

plt.subplot(122)
plt.title('Regressor', fontweight='bold')
pi_reg_scores = np.array(pi_reg['score'])
plt.barh(feature_names[np.argsort(pi_reg_scores)],
         pi_reg_scores[np.argsort(pi_reg_scores)])
plt.xlabel('Permutation Importance')
