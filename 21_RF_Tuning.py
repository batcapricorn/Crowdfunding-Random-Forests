...
os.chdir(wd_export)
X = df.loc[:, ['name_length', 'blurb_gradelevel', 'category', 'country', 'state', 
               'city', 'goal_usd', 'duration']]
...
#Classifier
    #% Grid Search
grid_classifier = {'n_estimators': [100, 200, 300, 400],
...
for g in ParameterGrid(grid_classifier):
    ...
    oob_scores_cla.append([g['n_estimators'], g['max_features'], g['criterion'], rf.oob_score_])
    # save if best
...
#Regressor
    #% Grid Search
grid_regressor = {'n_estimators': [100, 200, 300, 400],
...
for g in ParameterGrid(grid_regressor):
    ...
    oob_scores_reg.append([g['n_estimators'], g['max_features'], g['criterion'],  rf.oob_score_])
    # save if best
    if rf.oob_score_ > best_score:
        best_score = rf.oob_score_
        best_grid = g
