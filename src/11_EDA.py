...
#% Average stats
obs = (
    df.groupby("location_country")
    .mean()[["funding_rate", "success_dummy", "usd_pledged", "usd_goal"]]
    .reset_index()
)
...
obs = (
    df.groupby("location_country")
    .std()[["funding_rate", "usd_pledged", "usd_goal"]]
    .reset_index()
)
obs.columns = [
    "location_country",
...
ax = plt.scatter(
    np.log(country_stats["avg_usd_pledged"].replace(0, 1)),
...
sns.set(palette="viridis")
ax = plt.scatter(
    np.log(country_stats_top20["avg_usd_pledged"].replace(0, 1)),
...
for country in country_stats_top20["location_country"]:
    plt.annotate(
        ...
        xy=(
            np.log(
                country_stats_top20[country_stats_top20["location_country"] == country][
                    "avg_usd_pledged"
                ]
            ),
            np.log(
...
time_stats["relative_campaign_count"] = (
    time_stats["campaign_count"] / time_stats["campaign_count"].sum()
)
...
sns.set(palette="viridis")
ax = plt.plot(
    time_stats.index, np.log(time_stats["campaign_count"]), label="N° of Campaigns"
)
ax = plt.plot(
    time_stats.index,
...
ax = plt.plot(
    time_stats.index,
    np.log(time_stats["avg_usd_goal"]),
    label="Avg. Funding Goal in $",
    dashes=(2, 2),
)
ax = plt.plot(
    time_stats.index,
    np.log(time_stats["avg_usd_pledged"]),
    label="Avg. Pledged in $",
    dashes=(2, 2),
)
plt.xlabel("Year")
...
obs.columns = ["main_category", "std_funding_rate", "std_usd_pledged", "std_usd_goal"]
category_stats = category_stats.merge(obs, on="main_category", how="left")
...
sns.set(palette="viridis")
ax = plt.scatter(
    np.log(category_stats["avg_usd_pledged"].replace(0, 1)),
...
for category in category_stats.loc[:10, "main_category"]:
    plt.annotate(
        ...
        xy=(
            np.log(
                category_stats[category_stats["main_category"] == category][
                    "avg_usd_pledged"
                ]
            ),
            np.log(
