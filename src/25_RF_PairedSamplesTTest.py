...
# Feature groups
feature_groups = {
    "basic_information": ["goal_usd", "duration"],
    "descriptive_information": ["name_length", "blurb_gradelevel", "category"],
...
for group in feature_groups.keys():
    ...
    cla = RandomForestClassifier(
        n_estimators=400, criterion="gini", max_features="sqrt"
    )
    cv_results = cross_validate(
        cla, X[feature_groups[group]], y_cla, cv=30, scoring="accuracy"
    )
...
for group in feature_groups.keys():
    ...
    cv_results = cross_validate(
        reg, X[feature_groups[group]], y_reg, cv=30, scoring="r2"
    )
    pickle.dump(cv_results, open(export_name, "wb"))
...
sns.kdeplot(
    cv_results["reg_r2_descriptive_information"], label="Descriptive Information"
)
...
classifier_li = [
    "cla_all_information",
    "cla_basic_information",
    "cla_descriptive_information",
    "cla_geographic_information",
]
    ...
    for right_item in classifier_li:
        if left_item == right_item:
            ...
            t_value, p_value = stats.ttest_ind(
                cv_results[left_item], cv_results[right_item]
            )
            if p_value <= 0.01:
                result = str(t_value) + "***"
            elif p_value <= 0.05:
...
# Regression
regression_li = [
    "reg_r2_all_information",
    "reg_r2_basic_information",
    "reg_r2_descriptive_information",
    "reg_r2_geographic_information",
]
