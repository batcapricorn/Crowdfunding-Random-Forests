...
for link in soup.find_all("a"):
    if (
        ("Kickstarter" in str(link))
        and ("json" not in str(link))
        and ("zip" in str(link))
    ):
    ...
    try:
        ...
        for item in items:
            if counter == 1:
                ...
                df_file = pd.concat(
                    [df_file, pd.read_csv(zf.open(item))], ignore_index=True
                )
                counter += 1
...
for file in files:
    if counter == 1:
        ...
        df_rough = df_rough[
            df_rough["state"].isin(["successful", "failed"])
        ]  # only successful/failed projects are completed
...
for column in unix_timestamps:
    df_rough[column] = pd.to_datetime(
        df_rough[column].apply(lambda x: convert_unix_timestamp(x))
    )
...
for column in dict_columns:
    df_rough[column] = df_rough[column].apply(
        lambda x: convert_strings_to_dictionaries(x)
    )
...
df_rough["user_profile"] = df_rough["creator"].apply(
    lambda x: extract_additional_data(x, "user_profile")
)
df_rough["location_country"] = df_rough["location"].apply(
    lambda x: extract_additional_data(x, "location_country")
)
df_rough["location_state"] = df_rough["location"].apply(
    lambda x: extract_additional_data(x, "location_state")
)
df_rough["city"] = df_rough["location"].apply(
    lambda x: extract_additional_data(x, "city")
)
df_rough["main_category"] = df_rough["category"].apply(
    lambda x: extract_additional_data(x, "main_category")
)
df_rough["project_link"] = df_rough["urls"].apply(
    lambda x: extract_additional_data(x, "project_link")
)
df_rough["reward_link"] = df_rough["urls"].apply(
    lambda x: extract_additional_data(x, "reward_link")
)

df_rough["usd_goal"] = df_rough["goal"] * df_rough["static_usd_rate"]
df_rough["duration"] = pd.to_datetime(df_rough["deadline"]) - pd.to_datetime(
    df_rough["launched_at"]
)
...
df_rough.to_csv(
    "crowdfunding_rough_draft_v1.csv",
    index=False,
    header=True,
