...
df_model_data["name_length"] = [
    str(x).strip("!-?/\,.*^@'#:+~").replace("-", "").replace(":", "")
    for x in df.loc[:, "name"]
]
df_model_data["name_length"] = [
    [y for y in x.split(" ") if y != ""] for x in df_model_data.loc[:, "name_length"]
]
...
for category in ["category", "country", "state", "city"]:
    df_model_data.loc[:, category] = le.fit_transform(
        [str(x) for x in df_model_data.loc[:, category]]
    )
...
df_model_data["successful"] = (
    df.loc[:, "success_dummy"].replace("successful", 1).replace("failed", 0)
)
