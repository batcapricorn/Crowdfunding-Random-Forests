...
f1_cla = f1_score(
    df.loc[:, "successful"], [get_prediction(x) for x in cla_proba]
)  # 0.7535335555790205
recall_cla = recall_score(
    df.loc[:, "successful"], [get_prediction(x) for x in cla_proba]
)  # 0.7744085821473972
precision_cla = precision_score(
    df.loc[:, "successful"], [get_prediction(x) for x in cla_proba]
)  # 0.7337544065804935
...
error_rate_reg = accuracy_score(
    df.loc[:, "successful"], y_dummy_reg, normalize=True
)  # 0.6944504780268069
...
precision_reg = precision_score(
    df.loc[:, "successful"], y_dummy_reg
)  # 0.7429324872496017


# ROC Curve
fpr_cla, tpr_cla, thresholds_cla = roc_curve(df.loc[:, "successful"], cla_proba)
fpr_reg, tpr_reg, thresholds_reg = roc_curve(
    df.loc[:, "successful"], reg.oob_prediction_
)
...
# Stats
stats_dict = {
    "oob_score_rough": [oob_error_cla, oob_error_reg],
